...
const vertexShader = /* glsl */ `
  attribute float aType;      // 0 = spiral, 1 = debris, 2 = streamer
  attribute float aRandom;
  attribute float aProgress;  // 0-1 position along height
  attribute float aAngleBase; // initial angle offset
  ...
  void main() {
    ...
    // Debris: orbiting chunks flung outward
    } else if (aType < 1.5) {
    ...
    // Streamers: lightning-like bolts along the axis
    } else {
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Spiral: heatmap from base (cold) to top (hot/eye)
    if (vType < 0.5) {
      if (vHeight < 0.4) {
...
export default function Vortex({ settings }: Props) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < DEBRIS_COUNT; i++) {
      ...
      types[idx]      = 1;
      randoms[idx]    = Math.random();
      progresses[idx] = (y + HEIGHT * 0.5) / HEIGHT;
      angleBases[idx] = angle;
      idx++;
    }
